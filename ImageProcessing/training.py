...
def splitDataAndLabel(datas,labels,listUniqueLabel,percSplit):
    # split train test
    # percSplit = 70
    ...
    for iclass in listUniqueLabel:
        ...
        posTrain = random.sample(set(posClass[0,:]),nImgTrain)
        posClassFil = np.setdiff1d(posClass,posTrain)
        posValid = random.sample(set(posClassFil),nImgValid)
...
def loadLabeledImage(folderImg,imgType = '*.jpg',labeledType = 'folderName'):
    '''
    myFolder
      -ClassA
        -img1.jpg
    ...
    # image in 4D matric
    loadedImg = imreadFromPath(listAbs)
